...
class Order extends \yii\easyii\components\ActiveRecord
{
    ...
    public function rules()
    {
        return [
           ...
           // ['email', 'required', 'when' => function($model){ return $model->scenario == 'confirm' && Yii::$app->getModule('admin')->activeModules['orders']->settings['enableEmail']; }],
          //  ['phone', 'required', 'when' => function($model){ return $model->scenario == 'confirm' && Yii::$app->getModule('admin')->activeModules['orders']->settings['enablePhone']; }],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'address' => Yii::t('easyii/orders', 'Address'),
            'phone' => Yii::t('easyii/orders', 'Phone'),
            'comment' => Yii::t('easyii/orders', 'Comment'),
            'remark' => Yii::t('easyii/orders', 'Admin remark'),
        ];
    }
    ...
    {
        return [
            self::STATUS_BLANK => Yii::t('easyii/orders', 'Blank'),
            self::STATUS_PENDING => Yii::t('easyii/orders', 'Pending'),
            self::STATUS_PROCESSED => Yii::t('easyii/orders', 'Processed'),
            self::STATUS_DECLINED => Yii::t('easyii/orders', 'Declined'),
            self::STATUS_SENT => Yii::t('easyii/orders', 'Sent'),
            self::STATUS_RETURNED => Yii::t('easyii/orders', 'Returned'),
            self::STATUS_ERROR => Yii::t('easyii/orders', 'Error'),
            self::STATUS_COMPLETED => Yii::t('easyii/orders', 'Completed'),
        ];
    }
