...
use app\modules\contactus\models\Feedback;
...
class AController extends Controller
{
    ...
    public function init()
    {
        ...
        $this->new = Yii::$app->getModule('admin')->activeModules['contactus']->notice;
        ...
        if($postData) {
            if(filter_var(Setting::get('admin_email'), FILTER_VALIDATE_EMAIL))
            {
                ...
                if($model->sendAnswer()){
                    ...
                    $model->save();
                    $this->flash('success', Yii::t('easyii/contactus', 'Answer successfully sent'));
                }
                else{
                    $this->flash('error', Yii::t('easyii/contactus', 'An error has occurred while sending mail'));
                }
            }
            else {
                $this->flash('error', Yii::t('easyii/contactus', 'Please fill correct `Admin E-mail` in Settings'));
            }
            ...
            if(!$model->answer_text) {
                $model->answer_subject = Yii::t('easyii/contactus', $this->module->settings['answerSubject']);
                if ($this->module->settings['answerHeader']) $model->answer_text = Yii::t('easyii/contactus', $this->module->settings['answerHeader']) . " " . $model->name . ".\n";
                if ($this->module->settings['answerFooter']) $model->answer_text .= "\n\n" . Yii::t('easyii/contactus', $this->module->settings['answerFooter']);
            }
    ...
    public function actionSetAnswer($id)
    {
        ...
        if($model === null){
            ...
            $model->status = Feedback::STATUS_ANSWERED;
            if($model->update()) {
                $this->flash('success', Yii::t('easyii/contactus', 'Feedback updated'));
            }
            else{
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('easyii/contactus', 'Feedback deleted'));
    }
}